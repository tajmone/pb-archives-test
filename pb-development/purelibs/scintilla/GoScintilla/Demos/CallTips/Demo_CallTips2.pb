...
If OpenWindow(0, 100, 200, 740, 600, "GoScintilla demo!", #PB_Window_SystemMenu | #PB_Window_MinimizeGadget | #PB_Window_ScreenCentered)
  ...
  ;=======================================================
    ...
    ;We have placed the keywords/call-tips in a data section for convenience.
      ...
      For i = 0 To numKeywords-1
        ...
        Read.s callTips$(i)
      Next
      GOSCI_AddKeywordsEx(1, numKeywords, keywords$(), calltips$(), "("+Chr(34), ")"+Chr(34), #STYLES_FUNCTIONS, #GOSCI_ALLOWWHITESPACEPRECEEDINGOPENCALLTIP)

    ;We now mark () as null call-tip separators. This means that, for example, in Print(xxx()) where Print() has no call-tip, the
    ;closing paranthesis in xxx() does not cause Print()'s call-tip to close. Without this, then the Print() call-tip will close after
  ...
  ;======================
    ...
    text$ + "; (NB - only Print " + Chr(34) + Chr(34) +" and Print () support call-tips in this demo!)" + #CRLF$ + #CRLF$
    text$ + "; Like demo 1, we again mark () as global null call-tip separators." + #CRLF$ + #CRLF$
    text$ + "; Note how we combine #GOSCI_DELIMITBETWEEN delimiters with call-tip separators." + #CRLF$ + #CRLF$
...
DataSection
  ;Some keywords with call-tips using () and "" separators.
    Data.i 1
    Data.s "Print", "Print() or Print " + Chr(34) + Chr(34) + #LF$ + "Displays a string."
EndDataSection
